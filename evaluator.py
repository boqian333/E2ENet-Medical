...
def evaluate_folder(folder_with_gts: str, folder_with_predictions: str, labels: tuple, **metric_kwargs):
    ...
    test_ref_pairs = []
    for i in files_pred:
        pure_name = i.split('_0000')
        file_name = ''.join(pure_name)
        test_ref_pairs.append((join(folder_with_predictions, i), join(folder_with_gts, file_name)))
...
def change_files_name():
    input_path = '/nnUNet/imageTs_out_216_nnunet_pp_200'
    output_path = '/nnUNet/imageTs_out_216_E2Enet'
    files_in = subfiles(input_path, suffix=".nii.gz", join=False)
    ...
    for i in files_in:
        pure_name = i.split('_0000')
        file_name = ''.join(pure_name)
        shutil.copy(join(input_path, i), join(output_path, file_name))
