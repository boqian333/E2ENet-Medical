...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    e2enet.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    ...
    init = info['init']
    name = info['name']
    search_in = join(e2enet.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="e2enet.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in e2enet.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within e2enet.trainig.network_training (search is recursive)."
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best"):
    ...
    ## select network
    if 'shiftConvPP' in checkpoint_name:
        trainer.Tconv = checkpoint_name.split('_model')[0]
    else:
        trainer.Tconv = 'ori' ## shiftConvPP
...
if __name__ == "__main__":
    pkl = "../nnUnet/3d_fullres/Task004_Hippocampus/nnUNetTrainerV2__nnUNetPlansv2.1/fold_0/model_final_checkpoint.model.pkl"
    checkpoint = pkl[:-4]
    train = False
