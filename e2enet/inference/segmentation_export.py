...
from e2enet.preprocessing.preprocessing import get_lowres_axis, get_do_separate_z, resample_data_or_seg
...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    ## current_shape 是经过resample后的，所以需要根据crop后的开始大小再做一次resample；最后还需要返回crop前的大小
    ...
    if np.any([i != j for i, j in zip(np.array(current_shape[1:]), np.array(shape_original_after_cropping))]):
        ...
        # 该函数对segmentation_softmax(data在数据预处理进行了resample),进行resample成crop时的大小
