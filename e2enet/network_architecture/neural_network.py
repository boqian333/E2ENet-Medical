...
from e2enet.utilities.random_stuff import no_op
...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_2D(self, x, do_mirroring: bool, mirror_axes: tuple = (0, 1, 2), use_sliding_window: bool = False,
                   ...
                   use_gaussian: bool = False, pad_border_mode: str = "constant",
                   pad_kwargs: dict = None, all_in_gpu: bool = False,
                   verbose: bool = True, mixed_precision: bool = False) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        for m in range(mirror_idx):
            if m == 0:
                pred = self.inference_apply_nonlin(self(x)) ## here inference: self(x)
