...
class SwinMLPSys(SegmentationNetwork):
    ...
    def forward_features(self, x):
        ...
        def window_partition(x, window_size):
            ...
            if len(x_shape) == 5:
                B, D, H, W, C = x.shape
                x = x.view(B, D // window_size, window_size, H // window_size, window_size, W // window_size,
                           window_size, C)
                windows = x.permute(0, 1, 3, 5, 2, 4, 6, 7).contiguous().view(-1, window_size, window_size, window_size,
                                                                              C)
        ...
        def window_reverse(windows, window_size, x_shape):
            ...
            if len(x_shape) == 5:
                _, D, H, W, _ = x_shape
                B = int(windows.shape[0] / (D * H * W / window_size / window_size / window_size))
                x = windows.view(B, D // window_size, H // window_size, W // window_size, window_size, window_size,
                                 window_size, -1)
                x = x.permute(0, 1, 4, 2, 5, 3, 6, 7).contiguous().view(B, D, H, W, -1)
            ...
            def forward(self, x):
                ...
                if len(self.input_resolution) == 2:
                    ...
                    x_windows_heads = x_windows_heads.reshape(-1, self.num_heads * self.window_size * self.window_size,
                                                              C // self.num_heads)
                    spatial_mlp_windows = self.spatial_mlp(x_windows_heads)  # nW*B, nH*window_size*window_size, C//nH
                    spatial_mlp_windows = spatial_mlp_windows.view(-1, self.num_heads,
                ...
                if len(self.input_resolution) == 3:
                    ...
                    # Window/Shifted-Window Spatial MLP
                    x_windows_heads = x_windows.view(-1, self.window_size * self.window_size * self.window_size,
                                                     self.num_heads,
                                                     C // self.num_heads)
                    x_windows_heads = x_windows_heads.transpose(1,
                                                                2)  # nW*B, nH, window_size*window_size*window_size, C//nH
                    x_windows_heads = x_windows_heads.reshape(-1,
                                                              self.num_heads * self.window_size * self.window_size * self.window_size,
                                                              C // self.num_heads)
                    spatial_mlp_windows = self.spatial_mlp(x_windows_heads)  # nW*B, nH*window_size*window_size, C//nH
                    spatial_mlp_windows = spatial_mlp_windows.view(-1, self.num_heads,
                                                                   self.window_size * self.window_size * self.window_size,
                                                                   C // self.num_heads).transpose(1, 2)
                    spatial_mlp_windows = spatial_mlp_windows.reshape(-1,
                                                                      self.window_size * self.window_size * self.window_size,
                                                                      C)

                    # merge windows
                    spatial_mlp_windows = spatial_mlp_windows.reshape(-1, self.window_size, self.window_size,
                                                                      self.window_size, C)
                    shifted_x = window_reverse(spatial_mlp_windows, self.window_size, shifted_x.shape)  # B H' W' C
        ...
        class PatchMerging(nn.Module):
            ...
            def forward(self, x):
                ...
                if len(self.input_resolution) == 2:
                    ...
                    B, L, C = x.shape
                    assert L == H * W, "input feature has wrong size"
                    assert H % self.pool_strides[0] == 0 and W % self.pool_strides[
                        1] == 0, f"x size ({H}*{W}) are not even."
                ...
                if len(self.input_resolution) == 3:
                    ...
                    B, L, C = x.shape
                    assert L == D * H * W, "input feature has wrong size"
                    assert D % self.pool_strides[0] == 0 and H % self.pool_strides[1] == 0 and W % self.pool_strides[
                        2] == 0, f"x size ({D}*{H}*{W}) are not even."
                    ...
                    for i in range(self.pool_strides[2]):
                        for m in range(self.pool_strides[1]):
                            for n in range(self.pool_strides[0]):
                                x_concat.append(
                                    x[:, n::self.pool_strides[0], m::self.pool_strides[1], i::self.pool_strides[2], :])
        ...
        class PatchExpand(nn.Module):
            def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm, pool_stride=None):
                ...
                self.pool_stride = pool_stride
                if len(self.input_resolution) == 2:
                    self.expand = nn.Linear(dim, np.prod(np.array(self.pool_stride)) / 2 * dim,
                                            bias=False) if dim_scale == 2 else nn.Identity()
                if len(self.input_resolution) == 3:
                    a = np.prod(np.array(self.pool_stride)) / 2
                    self.expand = nn.Linear(dim, int(np.prod(np.array(self.pool_stride)) / 2) * dim,
                                            bias=False) if dim_scale == 2 else nn.Identity()
                self.norm = norm_layer(dim // dim_scale)
            ...
            def forward(self, x):
                ...
                if len(self.input_resolution) == 3:
                    ...
                    x = x.view(B, D, H, W, C)
                    x = rearrange(x, 'b d h w (p1 p2 p3 c)-> b (d p1) (h p2) (w p3) c',
                                  p1=self.pool_stride[0], p2=self.pool_stride[1], p3=self.pool_stride[2],
                                  c=C // np.prod(self.pool_stride))
                    x = x.view(B, -1, C // np.prod(self.pool_stride))
                    # print(x)
                x = self.norm(x)
        ...
        class FinalPatchExpand_X4(nn.Module):
            ...
            def forward(self, x):
                ...
                if len(self.input_resolution) == 2:
                    ...
                    x = x.view(B, D, H, W, C)
                    x = rearrange(x, 'b d h w (p1 p2 p3 c)-> b (d p1) (h p2) (w p3) c', p1=self.dim_scale[0],
                                  p2=self.dim_scale[1], p3=self.dim_scale[2],
                                  c=C // np.prod(np.array(self.dim_scale)))
                    x = x.view(B, -1, self.output_dim)
                x = self.norm(x)
                return x
        ...
        class BasicLayer_up(nn.Module):
            ...
            def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                         mlp_ratio=4., drop=0., drop_path=0., norm_layer=nn.LayerNorm, upsample=None,
                         use_checkpoint=False, pool_stride=None):
