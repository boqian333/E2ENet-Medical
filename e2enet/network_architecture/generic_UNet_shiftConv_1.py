...
class Generic_UNet_shiftConv(SegmentationNetwork):
    ...
    def __init__(self, img_size, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        self.seg_outputs = []
        self.conv_concat = []
        self.conv_concat_up = nn.ModuleList()
        ...
        for u in range(num_pool):
            ...
            self.conv_concat_up.append(nn.Conv3d(n_features_after_tu_and_concat + final_num_features, final_num_features, 1, 1, 0))
        ...
        self.S = [2**i for i in range(num_pool+1)]

        self.fc3 = nn.ModuleList()
        self.fc3_up = nn.ModuleList()
        for n in range(num_pool):
            if n == 0:
                ...
                self.fc3_up.append(nn.Conv3d(
                    input_resolution[-n-2][0] // input_resolution[-n-2][0] * input_resolution[-n-2][1] // 2 *
                    input_resolution[-n-2][2] // 2 * self.S[-n-2],
                    input_resolution[-n-2][0] // input_resolution[-n-2][0] * input_resolution[-n-2][1] // 2 *
                    input_resolution[-n-2][2] // 2 * self.S[-n-2], 1, 1, 0, bias=False, groups=self.S[-n-2]))
            else:
                self.fc3.append(
        ...
        self.fc3_bn_up = nn.ModuleList()
        for n in range(num_pool):
            if n == 0:
                self.fc3_bn.append(nn.BatchNorm3d(self.S[n]))
                self.fc3_bn_up.append(nn.BatchNorm3d(self.S[-n-2]))
            else:

                self.fc3_bn.append(nn.BatchNorm3d(self.S[n]))
                self.fc3_bn_up.append(nn.BatchNorm3d(self.S[-n - 2]))
        self.fc3_bn.append(nn.BatchNorm3d(self.S[num_pool]))
    ...
    def partition_affine(self, x, n_layer, S, d_parts, h_parts, w_parts, d, h, w, decode=False):
        fc_inputs = x.reshape(-1, S * d * h * w, 1, 1, 1)
        if decode:
            out = self.fc3_up[n_layer](fc_inputs)
            out = out.reshape(-1, S, d, h, w)
            out = self.fc3_bn_up[n_layer](out)
    ...
    def forward(self, x):
        ...
        fc3_out = self.partition_affine(partitions, len(self.conv_blocks_context) - 1, self.S[-1], 1, 1, 1,
                                        origin_shape[2], origin_shape[3], origin_shape[4], decode=False)
        ...
        for u in range(len(self.tu)):
            ...
            fc3_out = self.partition_affine(partitions, u, self.S[-u-2], d_parts, h_parts, w_parts, d, h, w, decode=True)
