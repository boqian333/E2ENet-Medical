...
def grid_reverse(windows, window_size, x_shape):
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
...
class SwinMLPBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=1., drop=0., drop_path=0.,
        ...
        for i in range(len(self.input_resolution)):
    ...
    def forward(self, x):
        ...
        if len(self.input_resolution) == 3:
            ...
            pad_d1 = (self.window_size[0]*2 - d % (self.window_size[0]*2)) % (self.window_size[0]*2)
            pad_b = (self.window_size[1]*2 - h % (self.window_size[1]*2)) % (self.window_size[1]*2)
            pad_r = (self.window_size[2]*2 - w % (self.window_size[2]*2)) % (self.window_size[2]*2)
        ...
        if len(self.input_resolution) == 3:
                ...
                # if len(self.input_resolution) == 3:
                #     P_d1, P_d2, P_l, P_r, P_t, P_b = [self.window_size[0] - self.shift_size[0], self.shift_size[0], self.window_size[1] - self.shift_size[1], self.shift_size[1],
                #                           self.window_size[2] - self.shift_size[2], self.shift_size[2]]  # P_l,P_r,P_t,P_b
            if self.shift_size[0] > 0:
                P_d1, P_d2 = self.window_size[0] - self.shift_size[0], self.shift_size[0]
            else:
                P_d1, P_d2 = 0, 0

            if self.shift_size[1] > 0:
                P_l, P_r = self.window_size[1] - self.shift_size[1], self.shift_size[1]
            else:
                P_l, P_r = 0,0

            if self.shift_size[2] > 0:
                P_t, P_b = self.window_size[2] - self.shift_size[2], self.shift_size[2]
            else:
                P_t, P_b = 0,0


            shifted_x = F.pad(x, [0, 0, P_t, P_b, P_l, P_r, P_d1, P_d2], "constant", 0)
        ...
        if len(self.input_resolution) == 3:
            ...
            # partition windows
            x_windows = grid_partition(shifted_x, [self.window_size[i]*2 for i in range(len(self.window_size))])  # nW*B, window_size, window_size, window_size, C
            x_windows_sizes = x_windows.shape
            ...
            spatial_mlp_windows = window_reverse(spatial_mlp_windows, self.window_size, x_windows_sizes)
            shifted_x = grid_reverse(spatial_mlp_windows, [self.window_size[i]*2 for i in range(len(self.window_size))], shifted_x.shape)# B H' W' C
            ...
            if P_d1 > 0:
                x = shifted_x[:, P_d1:-P_d2, :, :, :].contiguous()
            else:
                x = shifted_x[:, :, :, :, :].contiguous()

            if P_t > 0:
                x = shifted_x[:, :, P_t:-P_b, :, :].contiguous()
            else:
                x = shifted_x[:, :, :, :, :].contiguous()

            if P_l > 0:
                x = shifted_x[:, :, :, P_l:-P_r, :].contiguous()
            else:
                x = shifted_x[:, :, :, :, :].contiguous()
...
class BasicLayer(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., drop=0., drop_path=0., drop_path_rate=0.1,
        ...
        self.blocks = nn.ModuleList([
            SwinMLPBlock(dim=dim, input_resolution=input_resolution,
                         ...
                         shift_size=[0,0,0] if (i % 2 == 0) else [window_size[i] // 2 for i in range(len(window_size))], drop_path=dpr[i])
            for i in range(1)])
...
class Generic_UNet_shiftConv(SegmentationNetwork):
    ...
    def __init__(self, img_size, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        num_heads = [1]
        num_heads += [2**(i) for i in range(num_pool)]
        ...
        for d in range(num_pool):
            ...
            self.fc3.append(BasicLayer(input_features, input_resolution[d], 2, num_heads[d], window_size))
        ...
        window_size = [5, 11, 11]
        self.fc3.append(BasicLayer(input_features, input_resolution[num_pool], 2, num_heads[num_pool], window_size))
