        ...
        if self.dim_num == 3:
            ...
            for inx, xs_i in enumerate(xs_1):
                ...
                if inx == 0:
                    ...
                    shift = [torch.roll(x_c, shift, self.dim) for x_c, shift in zip(xs, range(-pad, pad + 1))]
                    x_shift = x_shift + shift

            x_cat = torch.cat(x_shift, 1)
            x_cat = torch.narrow(x_cat, self.dim, pad, (shape[self.dim]+pad_1)//2)
...
class AxialShift(nn.Module):
    ...
    def __init__(self, dim, shift_size, as_bias=True, proj_drop=0., num_dim=3, pixel=2):
        ...
        if self.num_dim == 3:
            ...
            self.conv2_1 = nn.Conv3d(dim*pixel, dim*pixel, (1, 1, 1), 1, 0, groups=1, bias=as_bias)
            self.conv2_2 = nn.Conv3d(dim*pixel, dim*pixel, (1, 1, 1), 1, 0, groups=1, bias=as_bias)
            self.conv2_3 = nn.Conv3d(dim*pixel, dim*pixel, (1, 1, 1), 1, 0, groups=1, bias=as_bias)
    ...
    def forward(self, x):
        ...
        if len(x.shape) == 5:
            ...
            x_shift_d1d2, pad_d1d2 = self.shift_dim2(x)

            x_lr = self.conv2_1(x_shift_lr)
            x_td = self.conv2_2(x_shift_td)
            x_d1d2 = self.conv2_3(x_shift_d1d2)

            x_lr = self.actn(x_lr)
            x_td = self.actn(x_td)
            x_d1d2 = self.actn(x_d1d2)
            ...
            x_td = x_td.reshape(B_, C, self.pixel, D, (H + pad_td) // self.pixel, W).permute(0, 1, 3, 4, 2, 5).reshape(B_, C, D, (H + pad_td), W)
            x_td = torch.narrow(x_td, 3, 0, H)
            x_d1d2 = x_d1d2.reshape(B_, C, self.pixel, (D + pad_d1d2) // self.pixel, H, W).permute(0, 1, 3, 2, 4, 5).reshape(B_, C, (D + pad_d1d2), H, W)
            x_d1d2 = torch.narrow(x_d1d2, 2, 0, D)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, spatial_dims=3, is_stem=True):
        super().__init__()
        if is_stem:
            kernel_size = 5
            padding = 2
        ...
        self.proj = Conv[Conv.CONV, spatial_dims](in_chans, embed_dim, kernel_size=kernel_size, stride=stride, padding=padding)
