...
def window_partition(x, window_size):
    ...
    if len(x_shape) == 5:
        B, D, H, W, C = x.shape
        x = x.view(B, window_size, D // window_size, H // window_size, window_size, W // window_size, window_size,  C)
        windows = x.permute(0, 2, 3, 5, 1, 4, 6, 7).contiguous().view(-1, window_size, window_size, window_size, C)
...
class Generic_UNet_shiftConv(SegmentationNetwork):
    ...
    def __init__(self, img_size, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for d in range(num_pool):
            ...
            self.fc3.append(SwinMLPBlock(input_features, input_resolution[d], num_heads[d]))
        ...
        self.fc3.append(SwinMLPBlock(input_features, input_resolution[num_pool], num_heads[num_pool]))
