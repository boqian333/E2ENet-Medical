...
class Generic_UNet_shiftConv(SegmentationNetwork):
    ...
    def __init__(self, img_size, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        if conv_op == nn.Conv2d:
            ...
            input_resolution = [(img_size[0], img_size[1], img_size[2]), (img_size[0], img_size[1], img_size[2])]
            input_resolution += list((img_size[0] // np.prod(np.array(pool_op_kernel_sizes)[:d+1,0]), img_size[1] // np.prod(np.array(pool_op_kernel_sizes)[:d+1,1]), img_size[2] // np.prod(np.array(pool_op_kernel_sizes)[:d+1,2])) for d in range(num_pool-1))
        ...
        for n in range(num_pool):
            if n == 0:
                self.fc3.append(nn.Conv3d(input_resolution[n][0]//(input_resolution[n][0]//4) *input_resolution[n][1]//2*input_resolution[n][2]//2*self.S[n], input_resolution[n][0]//(input_resolution[n][0]//4)*input_resolution[n][1]//2*input_resolution[n][2]//2*self.S[n], 1, 1, 0, bias=False, groups=self.S[n]))
            else:
                first_stride = pool_op_kernel_sizes[n - 1]
                self.fc3.append(
                    nn.Conv3d(input_resolution[n][0] // (input_resolution[n][0]//4) //2 * input_resolution[n][1] // 2 * input_resolution[n][2] // 2 * self.S[n],
                              input_resolution[n][0] // (input_resolution[n][0]//4) // first_stride[0] * input_resolution[n][1] // 2 // first_stride[1] * input_resolution[n][2] // 2 // first_stride[2] * self.S[n], 1, 1,
        ...
        self.fc3.append(nn.Conv3d(input_resolution[num_pool][0] * input_resolution[num_pool][1]* input_resolution[num_pool][2] * self.S[-1],
                                  input_resolution[num_pool][0] // first_stride[0]* input_resolution[num_pool][1]  // first_stride[1]* input_resolution[num_pool][2] // first_stride[2] * self.S[-1], 1,
                                  pool_op_kernel_sizes[-1], 0, bias=False, groups=self.S[-1]))
    ...
    def partition_affine(self, x, n_layer, S, d_parts, h_parts, w_parts, d, h, w, pool_kernel):
        ...
        out = out.reshape(-1, d_parts, h_parts, w_parts, S, d//pool_kernel[0], h//pool_kernel[1], w//pool_kernel[1])
    ...
    def forward(self, x):
        ...
        for n in range(len(self.conv_blocks_context) - 1):
            ...
            if n == 0:
            #   Channel Perceptron
                fc3_out = self.partition_affine(partitions, n, self.S[n], d_parts, h_parts, w_parts, d, h, w, (1,1,1))
                fc3_out = fc3_out.permute(0, 4, 1, 5, 2, 6, 3, 7)  # N, O, h_parts, out_h, w_parts, out_w
                out = fc3_out.reshape(*origin_shape)
            else:
                fc3_out = self.partition_affine(partitions, n, self.S[n], d_parts, h_parts, w_parts, d, h, w, self.pool_op_kernel_sizes[n-1])
                fc3_out = fc3_out.permute(0, 4, 1, 5, 2, 6, 3, 7)  # N, O, h_parts, out_h, w_parts, out_w
                #size = fc3_out.size
        ...
        fc3_out = self.partition_affine(partitions, len(self.conv_blocks_context) - 1, self.S[-1], 1, 1, 1,
                                        origin_shape[2], origin_shape[3], origin_shape[4], self.pool_op_kernel_sizes[-1])

        fc3_out = fc3_out.permute(0, 4, 1, 5, 2, 6, 3, 7)  # N, O, h_parts, out_h, w_parts, out_w
        out = fc3_out.reshape(origin_shape[0], origin_shape[1], origin_shape[2] // 2, origin_shape[3] // 2,
                              origin_shape[4] // 2)
