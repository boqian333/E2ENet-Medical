...
def window_partition(x, window_size):
    ...
    if len(x_shape) == 5:
        B, D, H, W, C = x.shape
        x = x.view(B, window_size[0], D // window_size[0],  H // window_size[1], window_size[1], W // window_size[2], window_size[2],  C)
        windows = x.permute(0, 2, 3, 5, 1, 4, 6, 7).contiguous().view(-1, window_size[0], window_size[1], window_size[2], C)
...
def window_reverse(windows, window_size, x_shape):
    ...
    if len(x_shape) == 5:
        ...
        x = x.permute(0, 4, 1, 2, 5, 3, 6, 7).contiguous().view(B, D, H, W, -1)
...
def grid_reverse(windows, window_size, x_shape):
    ...
    if len(x_shape) == 5:
        ...
        x = x.permute(0, 4, 1, 5, 2, 6, 3, 6, 7).contiguous().view(B, D, H, W, -1)
