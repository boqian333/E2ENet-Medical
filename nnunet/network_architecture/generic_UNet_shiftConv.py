...
class Generic_UNet_shiftConv(SegmentationNetwork):
    ...
    def __init__(self, img_size, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        #     if d == 0:
        #         self.fc3.append(nn.Conv3d(input_resolution[d][0]//8*input_resolution[d][1]//4*input_resolution[d][2]//4*self.S[d], input_resolution[d][0]//8*input_resolution[d][1]//4*input_resolution[d][2]//4*self.S[d], 1, 1, 0, bias=False, groups=self.S[d]))
        #     else:
        #         self.fc3.append(
        #             nn.Conv3d(input_resolution[d][0] // 8 * input_resolution[d][1] // 2 * input_resolution[d][2] // 2 * self.S[d],
        #                       input_resolution[d][0] // 8 * input_resolution[d][1] // 2 * input_resolution[d][2] // 2 * self.S[d], 1, 1,
        #                       0, bias=False, groups=self.S[d]))
        #
        ...
        #     if d == 0:
        #         self.fc3_bn.append(nn.BatchNorm3d(self.S[d]))
        #     else:
        #
        #         self.fc3_bn.append(nn.BatchNorm3d(self.S[d]))
    ...
    def forward(self, x):
        ...
        #
        # #   Channel Perceptron
        # fc3_out = self.partition_affine(partitions, len(self.conv_blocks_context) - 1, self.S[-1], 1, 1, 1, origin_shape[2], origin_shape[3], origin_shape[4])
        #
        # fc3_out = fc3_out.permute(0, 4, 1, 5, 2, 6, 3, 7)  # N, O, h_parts, out_h, w_parts, out_w
        # out = fc3_out.reshape(*origin_shape)
