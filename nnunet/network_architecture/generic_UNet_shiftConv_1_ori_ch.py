...
class Generic_UNet_shiftConv(SegmentationNetwork):
    ...
    def __init__(self, img_size, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for d in range(num_pool):
            ...
            self.conv_concat.append(nn.Conv3d(input_features//self.S[d]+output_features, output_features, 1, 1, 0))
        ...
        for n in range(num_pool):
            if n == 0:
                ...
                self.fc3.append(nn.Conv3d(input_resolution[n][0] // (input_resolution[n][0]//4) * input_resolution[n][1] // 2 * input_resolution[n][2] // 2 * self.S[n], input_resolution[n][0] // (input_resolution[n][0]//4) * input_resolution[n][1] // 2 * input_resolution[n][2] // 2, 1, 1, 0, bias=False, groups=1))
        ...
        self.fc3.append(nn.Conv3d(input_resolution[num_pool][0] * input_resolution[num_pool][1] * input_resolution[num_pool][2] * self.S[-1],
                                  ...
                                  1, 0, bias=False, groups=1))

        self.fc3_bn = nn.ModuleList()
        for n in range(num_pool+1):
            if n == 0:
                self.fc3_bn.append(nn.BatchNorm3d(1))
            else:
    ...
    def partition_affine(self, x, n_layer, S, d_parts, h_parts, w_parts, d, h, w):
        ...
        out = out.reshape(-1, 1, d, h, w)
        out = self.fc3_bn[n_layer](out)
        out = out.reshape(-1, d_parts, h_parts, w_parts, 1, d, h, w)
    ...
    def forward(self, x):
        ...
        for n in range(len(self.conv_blocks_context) - 1):
            ...
            fc3_out = fc3_out.permute(0, 4, 1, 5, 2, 6, 3, 7)  # N, O, h_parts, out_h, w_parts, out_w
            out = fc3_out.reshape(origin_shape[0], origin_shape[1] // self.S[n], origin_shape[2], origin_shape[3],
                                      origin_shape[4])
