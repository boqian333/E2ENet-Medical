...
# mlp+3D_CNN (3,3,3) without shiftConv
...
class SwinMLPBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.grid_size = [self.window_size[i]*2 for i in range(len(self.window_size))]
        for i in range(len(self.input_resolution)):
            if self.input_resolution[i] <= self.window_size[i]:
                ...
                self.grid_size[i] = self.input_resolution[i]
    ...
    def forward(self, x):
        ...
        if len(self.input_resolution) == 3:
            ...
            pad_d1 = (self.grid_size[0] - d % (self.grid_size[0])) % (self.grid_size[0])
            pad_b = (self.grid_size[1] - h % (self.grid_size[1])) % (self.grid_size[1])
            pad_r = (self.grid_size[2] - w % (self.grid_size[2])) % (self.grid_size[2])
        ...
        if len(self.input_resolution) == 3:
            ...
            # partition windows
            x_windows = grid_partition(shifted_x, self.grid_size)  # nW*B, window_size, window_size, window_size, C
            ...
            shifted_x = grid_reverse(spatial_mlp_windows, self.grid_size, shifted_x.shape)# B H' W' C
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinMLPBlock(dim=dim, input_resolution=input_resolution,
                         ...
                         shift_size=[0,0,0], drop_path=dpr[i])
            for i in range(1)])
...
class Generic_UNet_shiftConv(SegmentationNetwork):
    ...
    def __init__(self, img_size, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        window_size = [5, 11, 11]
        print("window_size: %s" % str(window_size))
