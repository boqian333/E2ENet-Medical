...
class ConvDropoutNormNonlin(nn.Module):
    ...
    def __init__(self, input_channels, output_channels,
        ...
        #self.branch1 = nn.Sequential(
                #     self.conv_op(input_channels, output_channels, kernel_size=1)
                # )
                # self.branch2 = nn.Sequential(
                #     self.conv_op(input_channels, input_channels, kernel_size=1),
                #     self.conv_op(input_channels, output_channels, kernel_size=(1, 1, 3), padding=(0, 0, 1)),
                #     self.conv_op(output_channels, output_channels, kernel_size=(1, 3, 1), padding=(0, 1, 0))
                # )
                # self.branch3 = nn.Sequential(
                #     self.conv_op(input_channels, input_channels, kernel_size=1),
                #     self.conv_op(input_channels, output_channels, kernel_size=(1, 1, 3),
                #                 padding=(0, 0, 1)),
                #     self.conv_op(output_channels, output_channels, kernel_size=(1, 3, 1),
                #                 padding=(0, 1, 0)),
                #     self.conv_op(output_channels, output_channels, kernel_size=(1, 1, 3),
                #                 padding=(0, 0, 1)),
                #     self.conv_op(output_channels, output_channels, kernel_size=(1, 3, 1), padding=(0, 1, 0))
                # )
...
class InceptionModule(nn.Module):
    #def __init__(self, in_ch, out1x1, out_nxn_reduce, out_nxn, double_nxn_reduce, double_nxn, poolproj, basic_block=ConvDropoutNormNonlin):
    def __init__(self, in_ch, out_ch, conv_op, norm_op, norm_op_kwargs, basic_block=ConvDropoutNormNonlin):
        super(InceptionModule, self).__init__()

        self.branch1 = nn.Sequential(
            basic_block(input_channels=in_ch, output_channels=out_ch, conv_op=conv_op, conv_kwargs = {'kernel_size': 1},norm_op=norm_op, norm_op_kwargs=norm_op_kwargs))

        self.branch2 = nn.Sequential(
            basic_block(input_channels=in_ch, output_channels=in_ch, conv_op=conv_op, conv_kwargs ={'kernel_size': 1},
        norm_op=norm_op, norm_op_kwargs=norm_op_kwargs),
            basic_block(input_channels=in_ch, output_channels=out_ch, conv_op=conv_op, conv_kwargs = {'kernel_size': (1, 1, 3), 'padding': (0, 0, 1)}, norm_op=norm_op, norm_op_kwargs=norm_op_kwargs),
            basic_block(input_channels=out_ch, output_channels=out_ch, conv_op=conv_op, conv_kwargs = {'kernel_size': (1, 3, 1), 'padding': (0, 1, 0)},norm_op=norm_op, norm_op_kwargs=norm_op_kwargs)
        )
        ...
        #     basic_block(input_channels=in_ch, output_channels=in_ch, conv_op=conv_op, conv_kwargs={'kernel_size': 1},
        # norm_op=norm_op, norm_op_kwargs=norm_op_kwargs),
        #     basic_block(input_channels=in_ch, output_channels=out_ch, conv_op=conv_op,
        #                 conv_kwargs={'kernel_size': (1, 1, 3), 'padding': (0, 0, 1)},
        # norm_op=norm_op, norm_op_kwargs=norm_op_kwargs),
        #     basic_block(input_channels=out_ch, output_channels=out_ch, conv_op=conv_op,
        #                 conv_kwargs={'kernel_size': (1, 3, 1), 'padding': (0, 1, 0)},
        # norm_op=norm_op, norm_op_kwargs=norm_op_kwargs),
        #     basic_block(input_channels=out_ch, output_channels=out_ch, conv_op=conv_op,
        #                 conv_kwargs={'kernel_size': (1, 1, 3), 'padding': (0, 0, 1)},
        # norm_op=norm_op, norm_op_kwargs=norm_op_kwargs),
        ...
        self.concat = nn.Conv3d(in_channels=3*out_ch, out_channels=out_ch, kernel_size=1)
...
class StackedConvLayers(nn.Module):
    def __init__(self, input_feature_channels, output_feature_channels, num_convs, input_resolution,
        ...
        self.blocks = InceptionModule(input_feature_channels, output_feature_channels, self.conv_op, self.norm_op, self.norm_op_kwargs)
...
class Generic_UNet_shiftConv(SegmentationNetwork):
    ...
    def __init__(self, img_size, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for n in range(num_pool):
            if n == 0:
                self.fc3.append(nn.Conv3d(input_resolution[n][0]//input_resolution[n][0]*input_resolution[n][1]//2*input_resolution[n][2]//2*self.S[n], input_resolution[n][0]//input_resolution[n][0]*input_resolution[n][1]//2*input_resolution[n][2]//2*self.S[n], 1, 1, 0, bias=False, groups=self.S[n]))
            else:
                self.fc3.append(
                    nn.Conv3d(input_resolution[n][0] // input_resolution[n][0] * input_resolution[n][1] // 2 * input_resolution[n][2] // 2 * self.S[n],
                              input_resolution[n][0] // input_resolution[n][0] * input_resolution[n][1] // 2 * input_resolution[n][2] // 2 * self.S[n], 1, 1,
                              0, bias=False, groups=self.S[n]))
    ...
    def partition_affine(self, x, n_layer, S, d_parts, h_parts, w_parts, d, h, w):
    ...
    def forward(self, x):
        ...
        for n in range(len(self.conv_blocks_context) - 1):
            ...
            #   Channel Perceptron
            fc3_out = self.partition_affine(partitions, n, self.S[n], d_parts, h_parts, w_parts, d, h, w)
