...
def add_sparse_args(parser):
    parser.add_argument('--sparse', type=str2bool, default=False, help='Enable sparse mode. Default: True.')
    ...
    parser.add_argument('--sparse_init', type=str, default='remain', help='sparse initialization: ERK, snip, Grasp')
    ...
    parser.add_argument('--update_frequency', type=int, default=50, metavar='N', help='how many iterations to train between parameter exploration')
    ...
    parser.add_argument('--keep_size', type=int, default=3, help='The pruning rate / death rate.')
...
class Masking(object):
    ...
    def init(self, mode='ERK', density=0.05, erk_power_scale=1.0):
        ...
        if mode == 'GMP':
            ...
            self.baseline_nonzero = 0
            for spe_initial, name in zip(keep_masks, self.masks):
                # print(spe_initial.shape)
                # print(self.masks[name].shape)
                assert (spe_initial.shape == self.masks[name].shape)
                self.masks[name] = spe_initial
                self.baseline_nonzero += self.masks[name].numel() * density

        elif mode == 'GraSP':
            print('initialize by GraSP')
            layer_wise_sparsities, keep_masks = GraSP(self.module, self.density, self.loader, self.device)
            # re-sample mask positions
            # for sparsity_, name in zip(layer_wise_sparsities, self.masks):
            #     self.masks[name][:] = (torch.rand(self.masks[name].shape) < (1-sparsity_)).float().data.cuda()
            ...
            first_layer_name = ''
            last_layer_name = ''
            for i, (name, weight) in enumerate(self.masks.items()):
            ...
            # dense_layers.add(first_layer_name)
            if self.last_dense:
                dense_layers.add(last_layer_name)
        ...
        if self.prune_every_k_steps is not None:
            ...
            if self.steps % self.prune_every_k_steps == 0:
                ...
                if self.explore_step > 1:
                    self.print_nonzero_counts()
                self.pre_masks = copy.deepcopy(self.regrowed_masks)
    ...
    def truncate_weights_kernel(self):
        # self.masks_reset()
        for module in self.modules:
            for name, weight in module.named_parameters():
                if name not in self.masks: continue
                ...
                self.name2nonzeros[name] = mask.sum().item()
                self.name2zeros[name] = mask.numel() - self.name2nonzeros[name]
                kernel_size = mask.shape[-1]

                # death
                if self.death_mode == 'magnitude':
                    # new_mask, remove_size = self.magnitude_death_kernel(mask, weight, name, ori_size=self.args.keep_size)
                    new_mask = self.magnitude_death(mask, weight, name)
                elif self.death_mode == 'SET':
                    new_mask = self.magnitude_and_negativity_death(mask, weight, name)
                elif self.death_mode == 'Taylor_FO':
                ...
                # self.num_remove[name] = remove_size
                self.masks[name][:] = new_mask
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                # growth
                if self.growth_mode == 'random':
                    new_mask = self.random_growth_tofix(name, new_mask, weight, ori_size=self.args.keep_size)
                    # new_mask = self.random_growth_kernel(name, new_mask, weight, ori_size=self.args.keep_size)
                ...
                if self.growth_mode == 'random_unfired':
                    ...
                    new_mask = self.gradient_growth_tofix(name, new_mask, weight, ori_size=self.args.keep_size)
    ...
    def print_nonzero_counts(self):
        for module in self.modules:
            for name, tensor in module.named_parameters():
                ...
                pruned_masks_neg = self.regrowed_masks[name].data < 1.0
                comp_1 = self.pre_masks[name].data.byte() & self.regrowed_masks[name].data.byte()
